...
int	main()
{
	...
	// clock_t begin = clock();
	t_list **map_array = get_map_array("maps/pyramide.fdf");
	...
	while (row)
	{
		...
		while (col)
		{
			...
			t_point	*p1 = rotate(x1, y1, item->deepth);
			t_point	*p2 = NULL;
			if (col->next)
			{
				x2 = x1 + space;
				...
				p2 = rotate(x2, y2, right_item->deepth);
				if (right_item->color)
					color = strtol(right_item->color, NULL, 0);
				mlx_draw_line(&img_data, p1->x + mx, p1->y + my, p2->x + mx, p2->y + my, color);
			}
			if (down_col)
			{
				x2 = x1;
				...
				p2 = rotate(x2, y2, down_item->deepth);
				if (down_item->color)
